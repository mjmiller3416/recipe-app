...
class DataManagementService:
    ...
    def export_full_backup(self) -> FullBackupDTO:
        ...
        shopping_states = self.session.query(ShoppingState).all()
            ...
            data=BackupDataDTO(
                ...
                shopping_items=[
                    ShoppingItemBackupDTO(
                        ...
                        have=si.have,
                        flagged=si.flagged,
                        state_key=si.state_key,
                        recipe_sources=si.recipe_sources,
                    )
    ...
    def preview_restore(self, backup: FullBackupDTO) -> RestorePreviewDTO:
        ...
        counts = {
            ...
            "shopping_states": len(backup.data.shopping_states),
        }
    ...
    def execute_restore(
        ...
        try:
            ...
            # 7. Restore shopping states
            for ss_dto in backup.data.shopping_states:
                new_ss = ShoppingState(
                    key=ss_dto.key,
                    quantity=ss_dto.quantity,
                    unit=ss_dto.unit,
                    checked=ss_dto.checked,
                    flagged=ss_dto.flagged,
                )
                self.session.add(new_ss)
            restored_counts["shopping_states"] = len(backup.data.shopping_states)

            # 8. Restore shopping items
            for si_dto in backup.data.shopping_items:
                new_si = ShoppingItem(
                    ...
                    state_key=si_dto.state_key,
                    recipe_sources=si_dto.recipe_sources,
                )
                self.session.add(new_si)
            restored_counts["shopping_items"] = len(backup.data.shopping_items)
