...
> **Archived — fully shipped.** All 5 phases below are implemented and verified against the
> current codebase (verified 2026-08-03: route groups, Home/GetStartedCard, MealCreationOverlay
> as the single meal-creation surface, deleted widgets, etc. all confirmed in place). Kept for
> the "why" behind decisions that aren't documented elsewhere — not a task list.
